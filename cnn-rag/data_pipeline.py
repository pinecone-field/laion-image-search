...
def delete_data():
    ...
    print(
        f"Deleted all vectors in index: {PINECONE_INDEX_NAME} for namespace: {PINECONE_NAMESPACE}"
    )


def main():
    parser = argparse.ArgumentParser(
        description="CLI for upserting and deleted pinecone index data"
    )
    parser.add_argument(
        "action",
        choices=["scrape_cnn", "upsert", "delete"],
        help="""Action to perform: 'scrape_cnn' to scrape data from CNN,
              'upsert' to insert or update data, 'delete' to delete all data in wbd namespace""",
    )
